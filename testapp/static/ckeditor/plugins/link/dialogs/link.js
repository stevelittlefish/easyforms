...
( function() {
	CKEDITOR.dialog.add( 'link', function( editor ) {
		...
		var internalLinkChanged = function() {
			var dialog = this.getDialog();
			...
			contents: [
				...
				elements: [
					...
					children: [
						...
						{
							// @@SB: here is the internal link magic!
							id: 'internalLink',
							type: 'select',
							label: 'Internal Link',
							'default': 'internal',
							items: [],
							onChange: internalLinkChanged,
							setup: function( data ) {
								var element_id = '#' + this.getInputElement().$.id;
								$.ajax('/blog/internal-links.js', {success: function(data){
									var dropdown = $(element_id);
									dropdown.html('<option value=""></option>');
									for (var i=0; i<data.length; i++) {
										dropdown.append('<option value="' + data[i][1] + '">' + data[i][0] + '</option>');
									}
								}})
							},
							commit: function( data ) {
